...
func Two() (string, string) {
	...
	//No native map function, doing this for now instead of jumping right into external libs
	numbers := make([]int, 0)

	for _, numberString := range numberStrings {
		intVal, _ := strconv.Atoi(string(numberString))
		numbers = append(numbers, intVal)
	}

	return strconv.Itoa(twoA(numbers, rowLength)), strconv.Itoa(twoB(numbers, rowLength))
}

func twoA(numbers []int, rowLength int) int {
	sum := 0

	for index := 0; index < len(numbers); index += rowLength {
		row := numbers[index : index+rowLength]

		var min, max int = math.MaxUint32, 0
...
func twoB(numbers []int, rowLength int) int {
	sum := 0

	for index := 0; index < len(numbers); index += rowLength {
		row := numbers[index : index+rowLength]

		for currentIndex, currentValue := range row {
